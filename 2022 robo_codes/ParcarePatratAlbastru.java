...
@Autonomous(name = "ParcarePatratAlbastru", group = "Autonomous")
public class ParcarePatratAlbastru extends LinearOpMode {
  ...
  public void runOpMode() throws InterruptedException {
   ...
   motorStangaSpate = hardwareMap.get(DcMotor.class, "motorStangaSpate");
	   ...
	   planetara = hardwareMap.get(DcMotor.class,"planetara2");
	 ...
	 if (opModeIsActive() && !isStopRequested())
	 {
	 	 //startPlanetara();
		 int caz;
		 if(CS1.getDistance(DistanceUnit.CM)<=8.5)
		 {
		 	cazJos();
		 }
		 else if(CS2.getDistance(DistanceUnit.CM)<=8.5)
		 {
   ...
   private void cazJos()
   {
	   ...
	   while(runtime.seconds() <= 10 && merge==0 && opModeIsActive())
	   {
		  ...
		  telemetry.update();
		  if(merge==0) merge=1;
		  motorManuta.setTargetPosition(-1330); //-1000  //-1350
		  ...
		  {   
			 ...
			 rotate(90,0.3);
			 driveForward(-45);
			 motorManuta.setTargetPosition(-1340); //-1000  //-1350
			 motorManuta.setPower(-0.25);
			 motorManuta.setMode(DcMotor.RunMode.RUN_TO_POSITION);
			 motorManuta.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
			 sleep(2200);
   ...
   private void cazMijloc()
   {
	   ...
	   while(runtime.seconds() <= 10 && merge==0 && opModeIsActive())
	   {
		  ...
		  {   
			 ...
			 rotate(35, 0.7); //se invarte inspre ciuperca // rotatiile cu minus la dreapta
			 driveForward(44); //duce manuta la ciuperca
			 ...
			 motorManuta.setPower(-0.4);
			 motorManuta.setMode(DcMotor.RunMode.RUN_TO_POSITION); // duc manuta inapoi jos
			 rotate(-79, 0.5); //se invarte spre rata
			 driveForward(77); //merge la rata
			 startPlanetara(); // invarte roata de la rata
			 driveForward(-10);
			 rotate(89,0.8); // se invarte spre parcare
			 driveForward(51); // se parcheaza
   ...
   private void cazSus()
   {
	   ...
	   while(runtime.seconds() <= 10 && merge==0 && opModeIsActive())
	   {
		  ...
		  {   
			 driveForward(-37); //merge in spate
			 rotate(33, 0.7); //se invarte inspre ciuperca // rotatiile cu minus la dreapta
			 driveForward(46); //duce manuta la ciuperca
			 deschideManuta(); // deschide ca sa lase cubul
			 driveForward(-20); //merge in spate
			 rotate(-54, 0.7); //se invarte spre rata
			 inchideManuta();
   ...
   {
   	   telemetry.addData("START", "PLANEtARA");
   	   telemetry.update();
	   ElapsedTime runtime = new ElapsedTime();
	   while(runtime.seconds() >= 0 && runtime.seconds() <= 6 && opModeIsActive())
		  planetara.setPower(-0.65);
