...
@Autonomous(name = "ParcareWarehouseRosu", group = "Autonomous")
public class ParcareWarehouseRosu extends LinearOpMode {
  ...
  public void runOpMode() throws InterruptedException {
    ...
    motorStangaSpate = hardwareMap.get(DcMotor.class, "motorStangaSpate");
	 ...
	 {
		 if(CS1.getDistance(DistanceUnit.CM) >= 31.5 && CS2.getDistance(DistanceUnit.CM) < 31.5)
		 {
		 	driveForward(40);
		 	cazJos();
		 }
		 else if(CS2.getDistance(DistanceUnit.CM) >= 31.5 && CS1.getDistance(DistanceUnit.CM) < 31.5)
		 {
		 	driveForward(40);
		 	cazMijloc();
		 }
		 else if(CS1.getDistance(DistanceUnit.CM) < 31.5 && CS2.getDistance(DistanceUnit.CM) < 31.5)
		 {
		 	driveForward(40);
		 	cazSus();
		 }
    ...
    private void cazJos()
    {
	   ...
	   while(runtime.seconds() <= 10 && merge==0 && opModeIsActive())
	   {
		  ...
		  {   
			 ...
			 rotate(30, 0.7); //se invarte inspre ciuperca // rotatiile cu minus la dreapta
			 driveForward(40); //duce manuta la ciuperca
			 deschideManuta(); // deschide ca sa lase cubul
			 driveForward(-10); //merge in spate
			 ...
			 rotate(-60, 0.7); //se invarte spre rata
			 driveForward(-25);
			 driveForward(170); //merge la rata
			 break;
		  }
	   }
	   motorManuta.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
    ...
    private void cazMijloc()
    {
	   ...
	   while(runtime.seconds() <= 10 && merge==0 && opModeIsActive())
	   {
		  ...
		  telemetry.update();
		  if(merge==0) merge=1;
		  motorManuta.setTargetPosition(-1400); // cat de mult se lasa manuta(cu cat valoarea ii mica cu atat ii mai jos, ex: -1500 mai jos decat -1300)
		  //motorManuta.setTargetPosition(1450);
		  motorManuta.setPower(-0.4);
		  motorManuta.setMode(DcMotor.RunMode.RUN_TO_POSITION);
    ...
    private void cazSus()
    {
	   ...
	   while(runtime.seconds() <= 10 && merge==0 && opModeIsActive())
	   {
		  ...
		  {   
			 driveForward(-37); //merge in spate
			 rotate(30, 0.7); //se invarte inspre ciuperca // rotatiile cu minus la dreapta
			 driveForward(43); //duce manuta la ciuperca
			 deschideManuta(); // deschide ca sa lase cubul
			 driveForward(-10); //merge in spate
			 rotate(-54, 0.7); //se invarte spre rata
			 motorManuta.setTargetPosition(300);
    ...
    void driveForwardParcare(int distance)
    {
	   double speedAfterAcceleratingRight = 0, speedAfterAcceleratingLeft = 0;
	   double acceleration = 0.01, deceleration = 0.01;
	   double diameter = 10;
