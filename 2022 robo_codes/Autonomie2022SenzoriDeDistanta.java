...
@Autonomous(name = "AutonomieDistanta", group = "Autonomous")
public class Autonomie2022SenzoriDeDistanta extends LinearOpMode {
  ...
  public void runOpMode() throws InterruptedException {
	...
	motorStangaSpate = hardwareMap.get(DcMotor.class, "motorStangaSpate");
	 ...
	 if (opModeIsActive() && !isStopRequested())
	 {
	  ...
	  if(CS1.getDistance(DistanceUnit.CM)<=5.5)
	  {
		 RataStanga = 1;
		 telemetry.addData("Caz", "Rata stanga");
		 telemetry.update();
	  }
	  else  if(CS2.getDistance(DistanceUnit.CM)<=5.5 && RataStanga==0)
	  {
		 RataMijloc = 1;
	...
	private void cazJos()
	{
	   ...
	   int merge = 0;
	   while(runtime.seconds() <= 29 && merge==0 && opModeIsActive())
	   {
		  if(merge==0) merge=1;
		  ...
		  {   
			 ...
			 rotate(-111, 0.7); //se invarte spre parcare
			 driveForward(43); //se parcheaza
			 motorManuta.setTargetPosition(300);
			 motorManuta.setPower(-0.4);
			 motorManuta.setMode(DcMotor.RunMode.RUN_TO_POSITION);
			 break;
	...
	private void cazMijloc()
	{
	   ...
	   while(runtime.seconds() <= 10 && merge==0 && opModeIsActive())
	   {
		  ...
		  {   
			 ...
			 startPlanetara();
			 driveForward(-7); //merge in spate
			 rotate(-96, 0.7); //se invarte spre parcare
			 driveForward(43); //se parcheaza
			 motorManuta.setTargetPosition(300);
			 motorManuta.setPower(-0.4);
			 motorManuta.setMode(DcMotor.RunMode.RUN_TO_POSITION);
	...
	private void cazSus()
	{
	   ...
	   while(runtime.seconds() <= 10 && merge==0 && opModeIsActive())
	   {
		  ...
		  {   
			 driveForward(-37); //merge in spate
			 rotate(-40, 0.7); //se invarte inspre ciuperca
			 driveForward(52); //duce manuta la ciuperca
			 deschideManuta();
			 driveForward(-20); //merge in spate
			 rotate(48, 0.7); //se invarte spre rata
			 motorManuta.setTargetPosition(300);
			 ...
			 motorManuta.setMode(DcMotor.RunMode.RUN_TO_POSITION);
			 rotate(95, 0.7); //se invarte spre rata
			 driveForward(75); //merge la rata
			 startPlanetara();
			 driveForward(-7); //merge in spate
			 rotate(-100, 0.7); //se invarte spre parcare
			 driveForward(45); //se parcheazamotorManuta.setTargetPosition(300);
			 motorManuta.setPower(-0.4);
			 motorManuta.setMode(DcMotor.RunMode.RUN_TO_POSITION);
			 motorManuta.setTargetPosition(300);
			 motorManuta.setPower(-0.4);
